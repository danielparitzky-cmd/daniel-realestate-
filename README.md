...
צבע הסטטוס של הלקוח חל בכל מקום שהוא מופיע — רשימת הלקוחות, מתעניינים בתוך נכס, ולוח השנה.
המקור היחיד הוא `BUYER_STATUS` ב-`src/lib/constants.ts`.
...
בלוח השנה הצבע שונה ומכוון: **אדום = טרם חזרת**, **ירוק = חזרת**. הקוביה נשארת על היום
שנקבע — הסימון מחליף צבע בלבד, לא מזיז אותה.
